 ...
 * Copyright 2016 TangoCash mod for Samsung SPF PhotoFrames
 */
 ...
 * Driver name
 */
#define VIRT_FB_NAME      "virtual_spf_fb"

static int vfbcount = 1;
static int vfbwidth = 800;
static int vfbheight = 480;
module_param(vfbcount, int, 0);
module_param(vfbwidth, int, 0);
module_param(vfbheight, int, 0);
...
static int virtfb_register(struct fb_info *fbi, unsigned int id)
{
	...
	sprintf(fbi->fix.id, "virtual_spf_fb%d", id);

	//Setup small default resolution
	fbi->var.xres_virtual = fbi->var.xres = vfbwidth;
	fbi->var.yres_virtual = fbi->var.yres = vfbheight;
...
MODULE_AUTHOR("TangoCash");
MODULE_DESCRIPTION("Virtual SPF framebuffer driver");
MODULE_LICENSE("GPL");
