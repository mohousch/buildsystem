...
int32_t SwPack::createImage(uint8_t **data)
{
	...
	*data = (uint8_t *)malloc(mDataLength);     //dbo
	//mData = (uint8_t *)malloc(mDataLength);   //dbo
	memcpy(*data, mHeader, sizeof(tSWPack));    //dbo
	//memcpy(mData, mHeader, sizeof(tSWPack));  //dbo
	for (uint32_t i = 0; i < this->mInventoryCount; i++)
	{
		...
		//memcpy(mData + SW_UPDATE_HEADER_SIZE + i * SW_INVENTORY_SIZE, cData, cDataLenght);    //dbo
		memcpy(*data + SW_UPDATE_HEADER_SIZE + i * SW_INVENTORY_SIZE, cData, cDataLenght);      //dbo
		free(cData);
		cDataLenght = this->mInventory[i]->getChildData(NULL);
		cData = (uint8_t *)malloc(cDataLenght);
		this->mInventory[i]->getChildData(&cData);
		//memcpy(mData + this->mInventory[i]->getImageOffset() + SW_UPDATE_HEADER_SIZE + MAX_INVENTORY_COUNT * SW_INVENTORY_SIZE, cData, cDataLenght); //dbo
		memcpy(*data + this->mInventory[i]->getImageOffset() + SW_UPDATE_HEADER_SIZE + MAX_INVENTORY_COUNT * SW_INVENTORY_SIZE, cData, cDataLenght);   //dbo
		free(cData);
		this->mInventory[i]->setChildDataFree();     //dbo
	}
	//*data = (uint8_t *)malloc(this->mDataLength);  //dbo
	//memcpy(*data, this->mData, this->mDataLength); //dbo
	//free(this->mData);    //dbo
	return mDataLength;
}
