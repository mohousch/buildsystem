...
static u8 tda10023_readreg(struct tda10023_state *state, u8 reg)
{
	...
	dprintk(50, "%s: reg=0x%02x , result=0x%02x\n",__func__, reg, b1[0]);
	return b1[0];
}
...
static int tda10023_writereg(struct tda10023_state *state, u8 reg, u8 data)
{
	...
	dprintk(50, "w: addr = 0x%02x : reg = 0x%02x val = 0x%02x\n", state->config->demod_address, reg, data);
...
/* fixme: use gate on here */
	...
	if (paramDebug >= 100)
	{
		...
		dprintk(1, "Writing %d bytes:", size);
		for (i = 0; i < size; i++)
		{
...
static int tda10023_init (struct dvb_frontend *fe)
{
	...
	u8 tda10023_inittab[] =
	{
		...
		/* 045 */ 0x2e, 0xb0, 0x30,  /* AGCCONF2 TRIAGC=0,POSAGC=ENAGCIF=1, PPWMTUN=0 PPWMIF=0 */
		...
		/* 093 */ 0x12, 0xff, TDA10023_OUTPUT_MODE_PARALLEL_B, /* INTP1 POCLKP=1 FEL=1 MFS=0 */
...
static int tda10023_get_frontend(struct dvb_frontend *fe, struct dvb_frontend_parameters *p)
{
	...
	dprintk(20, "DVB: TDA10023(%d): %sAFC (%d) %d Hz%s\n",
		state->frontend.dvb->num, sync & 2 ? "":"[", afc,
		-((s32)p->u.qam.symbol_rate * afc) >> 10, sync & 2 ? "":"]");
