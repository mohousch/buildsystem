...
static int stv6110x_read_reg(struct stv6110x_state *stv6110x, u8 reg, u8 *data)
{
	...
	if (ret != 2)
	{
		dprintk(1, "%s: I/O Error %d, addr=0x%02x, reg=0x%02x (res 0x%02x - 0x%02x) ret = %d (expected 2)\n", __func__, ret, config->addr, reg, *data, b1[0], ret);
		return -EREMOTEIO;
	}
	*data = b1[0];
...
static int stv6110x_write_reg(struct stv6110x_state *stv6110x, u8 reg, u8 data)
{
	...
	if (ret != 1)
	{
		dprintk(1, "%s: I/O Error %d, 0x%x\n", __func__, ret, config->addr);
		return -EREMOTEIO;
	}
	if (stv6110x->fe->ops.i2c_gate_ctrl)
	{
...
static int stv6110x_write_init(struct stv6110x_state *stv6110x)
{
...
#ifdef write_at_once
	{
		...
		if (ret != 1)
		{
			dprintk(1, "%s I/O Error %d, 0x%x, i2c-%d\n", __func__, ret, config->addr, stv6110x->i2c->nr);
			return -EREMOTEIO;
		}
...
#else
	{
		...
		dprintk(100, "%s > 0x%x, i2c-%d >\n", __func__, config->addr, stv6110x->i2c->nr);
...
static int stv6110x_set_frequency(struct dvb_frontend *fe, u32 frequency)
{
	...
	dprintk(100, "%s >\n", __func__);
	dprintk(20, "%s, freq=%d kHz, mclk=%d MHz\n", __func__, frequency, stv6110x->config->refclk / 1000000);
...
static int stv6110x_set_refclock(struct dvb_frontend *fe, u32 refclock)
{
	...
	dprintk(100, "%s > refclock = %d kHz\n", __func__, refclock / 1000);
	/* setup divider */
	switch (refclock)
	{
		default:
...
#if 0
static int stv6110x_get_state(struct dvb_frontend *fe, enum tuner_param param, struct tuner_state *state)
{
	switch (param)
	{
...
static int stv6110x_set_state(struct dvb_frontend *fe, enum tuner_param param, struct tuner_state *tstate)
{
	struct stv6110x_state *stv6110x = fe->tuner_priv;
...
struct tuner_devctl *stv6110x_attach(struct dvb_frontend *fe, const struct stv6110x_config *config, struct i2c_adapter *i2c)
{
	...
	dprintk(100, "%s > bus: i2c-%d -> 0x%p\n", __func__, i2c->nr, i2c);
	stv6110x->i2c    = i2c;
	stv6110x->config = config;
	dprintk(100, "refclk %d kHz\n", stv6110x->config->refclk / 1000);
